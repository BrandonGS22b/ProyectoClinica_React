...
function TableOrdenMedicamento(props) {
  const { OrdenesMedicamento, onView, onCancel } = props;
  ...
  useEffect(() => {
    const timeout = setTimeout(() => {
      if (!OrdenesMedicamento || OrdenesMedicamento.length === 0) {
  ...
  if (!showLoading || !OrdenesMedicamento || OrdenesMedicamento.length === 0) {
    return (
  ...
  let filteredOrdenes = OrdenesMedicamento;

  if (session.rol !== 'Medico' && session.rol !== 'Paciente') {
    filteredOrdenes = OrdenesMedicamento.filter(orden => {
...
export default TableOrdenMedicamento;